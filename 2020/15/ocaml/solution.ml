...
let solve starting_numbers turn_count =
    ...
    List.iteri (fun i n -> number_to_turn.(n) <- Int32.of_int i) input;
    ...
    for turn = List.length starting_numbers to pred turn_count do
        let last_spoken_on_turn = Int32.to_int number_to_turn.(!last_spoken) in
        number_to_turn.(!last_spoken) <- turn |> pred |> Int32.of_int;
